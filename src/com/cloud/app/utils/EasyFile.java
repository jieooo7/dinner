...
public class EasyFile {
	 ...
	 *@return 写入或者不写入
	 */
	public static boolean writeFile(String filePath, List<Map<String, String>> list) {
		String fileContent="";
//		创建文件目录，没有则创建
...
//		判断文件内容是否相等，否，则写入
			if((fileContent.equals(readFileStr(filePath)))!=true){
				return FileHelper.writeFile(FileManager.getSaveFilePath()+"json/"+filePath, fileContent, false);
				}
			else{
				return false;
			}
	...
	public static List<Map<String, String>> readFile(String filePath){
		...
		InputStream is=FileHelper.readFile(FileManager.getSaveFilePath()+"json/"+filePath);
		String st="";
		try {
